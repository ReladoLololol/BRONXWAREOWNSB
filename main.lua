...
--// 🛡️ SECURITY BLOCK
local BannedUsers = { 
    8691047775 -- This user is banned
}
...
--// 🎨 UI INITIALIZATION
if CG:FindFirstChild("BronxWare_V29") then CG.BronxWare_V29:Destroy() end
local SG = Instance.new("ScreenGui", CG); SG.Name = "BronxWare_V29"
...
MakeTab("COMBAT", UDim2.new(0.05, 0, 0, 5), UDim2.new(0.28, 0, 1, 0), CombatPage)
MakeTab("VISUALS", UDim2.new(0.36, 0, 0, 5), UDim2.new(0.28, 0, 1, 0), VisualPage)
    ...
    pcall(function()
        ...
        for _, v in pairs(Players:GetPlayers()) do
            if v ~= LP and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- Name & Dist ESP
                if _G.ESP_Name then
                    local h = hrp:FindFirstChild("BW_HUD") or Instance.new("BillboardGui", hrp)
                    h.Name = "BW_HUD"; h.AlwaysOnTop = true; h.Size = UDim2.new(0, 200, 0, 50); h.StudsOffset = Vector3.new(0, 3, 0)
                    local l = h:FindFirstChild("Main") or Instance.new("TextLabel", h)
                    l.Name = "Main"; l.Size = UDim2.new(1,0,0,20); l.BackgroundTransparency = 1; l.TextColor3 = isWL and Color3.new(0,1,0) or Color3.new(1,1,1)
                ...
                -- Inventory ESP (Stacked)
                if _G.ESP_Inv then
                    local iH = hrp:FindFirstChild("BW_INV") or Instance.new("BillboardGui", hrp)
                    iH.Name = "BW_INV"; iH.AlwaysOnTop = true; iH.Size = UDim2.new(0, 200, 0, 100); iH.StudsOffset = Vector3.new(0, -1.5, 0)
                    local cont = iH:FindFirstChild("List") or Instance.new("Frame", iH)
                    cont.Name = "List"; cont.Size = UDim2.new(1,0,1,0); cont.BackgroundTransparency = 1
                    local lay = cont:FindFirstChild("LL") or Instance.new("UIListLayout", cont); lay.Name = "LL"; lay.HorizontalAlignment = "Center"
                    for _, old in pairs(cont:GetChildren()) do if old:IsA("TextLabel") then old:Destroy() end end
                    
                    local eq = char:FindFirstChildOfClass("Tool")
                    local bp = v:FindFirstChild("Backpack")
                    if bp then
                        for _, item in pairs(bp:GetChildren()) do
                            local il = Instance.new("TextLabel", cont)
                            il.Size = UDim2.new(1,0,0,14); il.BackgroundTransparency = 1; il.TextColor3 = Color3.new(0.8,0.8,0.8); il.Text = item.Name; il.Font = "Gotham"; il.TextSize = 11
                        end
                    end
                    if eq then
                        local el = Instance.new("TextLabel", cont); el.LayoutOrder = -1
                        el.Size = UDim2.new(1,0,0,14); el.BackgroundTransparency = 1; el.TextColor3 = Color3.new(1,0.2,0.2); el.Text = eq.Name .. " (EQ)"; el.Font = "GothamBold"; el.TextSize = 11
                    end
                elseif hrp:FindFirstChild("BW_INV") then hrp.BW_INV:Destroy() end

                -- Highlight ESP
                if _G.ESP_Skel then
                    local highlight = char:FindFirstChild("BW_H") or Instance.new("Highlight", char)
                    highlight.Name = "BW_H"; highlight.FillTransparency = 0.5; highlight.FillColor = isWL and Color3.new(0,1,0) or Color3.fromRGB(200,0,0)
